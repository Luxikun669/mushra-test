...
FinishPage.prototype.sendResults = function() {
  // 1. 尝试发送给服务器 (保留原有逻辑，防止报错)
  // 如果你配置了 writeResults: false，这里其实不会做什么，但保留着也没坏处
  if (this.dataSender && this.dataSender.send) {
      this.dataSender.send(this.session);
  }

  // 2. 【核心新增】强制触发本地 CSV 下载
  // webMUSHRA 的全局对象通常挂载在 window.webMUSHRA 或 window._webMUSHRA 上
  // 我们调用它内置的导出功能
  console.log("Attempting to download results locally...");
  
  try {
      if (window._webMUSHRA && window._webMUSHRA.data && window._webMUSHRA.data.exportCSV) {
          window._webMUSHRA.data.exportCSV();
      } else if (window.webMUSHRA && window.webMUSHRA.data && window.webMUSHRA.data.exportCSV) {
          window.webMUSHRA.data.exportCSV();
      } else {
          // 如果找不到内置导出器，我们手动把 Session 转成 JSON 下载 (保底方案)
          console.warn("CSV exporter not found, falling back to JSON download.");
          var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(this.session));
          var downloadAnchorNode = document.createElement('a');
          downloadAnchorNode.setAttribute("href", dataStr);
          downloadAnchorNode.setAttribute("download", "experiment_backup.json");
      ...
      alert("自动下载失败。请按 F12 打开控制台，输入 '_webMUSHRA.data.exportCSV()' 手动下载。");
  }
